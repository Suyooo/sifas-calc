...
for F in vendor/*.css; do
    echo "    ${F}"
    purifycss ${F} index.html js/ui.js js/common.js vendor/materialize.js -m -o build/${F}
done
...
find build -not -path '*/\.*' -not -iname '.htaccess' -not -iname 'serviceworker.js' -not -iname 'networkinfo.js' -not -iname 'event_jp.png' -type f -printf '"%P"\n' | tr '\n' ',' | sed 's/,$//' >> build/serviceworker.js
echo "]);" >> build/serviceworker.js
